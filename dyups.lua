...
local function get_lock()
    ...
    if not ok then
        ...
        ngx_log(ngx_ERR, 'failed to add key "', key, '": ', err)
        return nil
    end
    return true
end
...
local function dump_tofile(force)
    ...
    while not saved do
        ...
        if l then
            ...
            if file == nil then
                ngx_log(ngx_ERR, "Can't open file: " .. f_path .. err)
                release_lock()
                return false
            end
...
local function spawn_healthcheck(upstream)
    ...
    if not _M.data[upstream].healthcheck then
        ...
        ngx_log(ngx_INFO, "upstream:" .. upstream .. " use default healthcheck")
    end
...
local function hash_server(name)
    ...
    if count == 0 then
        _M[name] = nil
        ngx_log(ngx_ERR, "upstream:", name, " not have a node")
        return
    end
    if not _M[name] then
        _M[name] = {}
        ...
        if not _M[name]["chash"] then
            ngx_log(ngx_ERR, "chash init err:", err)
        end
        _M[name]["rr"], err = resty_rr:new(server_list)
        if not _M[name]["rr"] then
            ngx_log(ngx_ERR, "rr init err:", err)
        end
        ngx_log(ngx_INFO, "balancer init:" .. name)
    else
        _M[name]["chash"]:reinit(server_list)
        ngx_log(ngx_INFO, "chash reinit:" .. name)
        _M[name]["rr"]:reinit(server_list)
        ngx_log(ngx_INFO, "rr reinit:" .. name)
    end
end
    ...
    for k, v in pairs(_M.data) do
        if type(v) == "table" then
            ...
            if not ok then
                ...
                ngx_log(ngx_INFO, "start watch from file:", k)
            end
            hash_server(k)
            ngx_log(ngx_INFO, "hash " .. k .. " due to start from file")
        end
    end
end
...
function _M.find(name, key, hash_method)
    if not _M[name] then
        ngx_log(ngx_ERR, "upstream:", name, " haven't hash")
        return nil, "upstream:" .. name .. " haven't hash"
    end
    local method = hash_method
    if not method or (method ~= "chash" and method ~= "rr") then
        ngx_log(ngx_ERR, "invalid hash method:", method, " use default hash method: chash")
        method = "chash"
    end
    local hash = _M[name][method]
    if not hash then
        ngx_log(ngx_ERR, "can't find ", name, "'s hash handle:", method)
...
local function watch(premature, conf, index)
    ...
    ngx_log(ngx_INFO, "index:", index)

    -- First time to init all the upstreams.
    if index == nil then
        -- ngx_log(ngx_ERR,"DEBUG: recieve change: "..body)
            ...
            if not err then
                ...
                if not all.errorCode and all.node.nodes then
                    for n, s in pairs(all.node.nodes) do
                            ...
                            if not err then
                                ...
                                if not svc.errorCode and svc.node.nodes then
                                    for i, j in pairs(svc.node.nodes) do
                                        ...
                                        if not err then
                                            _M.data[name].servers[#_M.data[name].servers + 1] = {
                                                host = h,
                                                port = p,
                                                weight = w,
                                                status = s
                        ...
                        ngx_log(ngx_INFO, "start init healthcheck:", name)
                        hash_server(name)
                        ngx_log(ngx_INFO, "chash " .. name .. " due to start from etcd")
                    end
                end
                _M.ready = true
                ...
                if not change.errorCode then
                    ...
                    if change.node.dir then
                        ...
                        if action == "delete" then
                            _M.data[target] = nil
                            ngx_log(ngx_INFO, "DELETE [" .. target .. "]")
                        elseif action == "set" or action == "update" then
                            ...
                            if not _M.data[new_svc] then
                                ...
                                ngx_log(ngx_INFO, "ADD [" .. new_svc .. "]")
                                local ok, err = spawn_healthcheck(new_svc)
                                if not ok then
                                    ngx_log(ngx_ERR, err)
                                end
                                ngx_log(ngx_INFO, "start dir watch healthcheck:" .. new_svc)
                            end
                        end
                    else
                        ...
                        if not err then
                            ...
                            if action == "delete" or action == "expire" then
                                ...
                                if _M.data[svc].up_servers[h .. ":" .. p] then
                                    ...
                                    ngx_log(ngx_INFO, "DELETE up_servers [" .. svc .. "]: " .. h .. ":" .. p)
                                    hash_server(svc)
                                    ngx_log(ngx_INFO, "chash " .. svc .. " due to DELETE " .. h .. ":" .. p)
                                end
                                ngx_log(ngx_INFO, "DELETE [" .. svc .. "]: " .. bs.host .. ":" .. bs.port)
                            elseif action == "set" or action == "update" then
                                if not _M.data[svc] then
                                    ...
                                    ngx_log(ngx_INFO, "ADD [" .. svc .. "]: " .. bs.host .. ":" .. bs.port)
                                    if s == "up" then
                                        _M.data[svc].up_servers[h .. ":" .. p] = w
                                        ngx_log(ngx_INFO, "ADD up_servers [" .. svc .. "]: " .. h .. ":" .. p)
                                        hash_server(svc)
                                        ngx_log(ngx_INFO, "hash " .. svc .. " due to DIR ADD " .. h .. ":" .. p)
                                    end
                                    local ok, err = spawn_healthcheck(svc)
                                    if not ok then
                                        ngx_log(ngx_ERR, err)
                                    else
                                        ngx_log(ngx_INFO, "start healthcheck due to DIR ADD:" .. svc)
                                    end
                                else
                                    ...
                                    if index == nil then
                                        ...
                                        if s == "up" then
                                            ...
                                            ngx_log(ngx_INFO, "ADD up_servers [" .. svc .. "]: " .. h .. ":" .. p)
                                            hash_server(svc)
                                            ngx_log(ngx_INFO, "hash " .. svc .. " due to KEY ADD " .. h .. ":" .. p)
                                        end
                                    else
                                        ...
                                        ngx_log(
                                            ngx_INFO,
                                            "MODIFY [" ..
                                                svc .. "]: " .. bs.host .. ":" .. bs.port .. " " .. change.node.value
                                        )
                                        if s == "up" then
                                            _M.data[svc].up_servers[h .. ":" .. p] = w
                                            ngx_log(
                                                ngx_INFO,
                                                "MODIFY ADD up_servers [" .. svc .. "]: " .. h .. ":" .. p
                                            )
                                            hash_server(svc)
                                            ngx_log(ngx_INFO, "hash " .. svc .. " due to MODIFY " .. h .. ":" .. p)
                                        elseif s == "down" then
                                            if _M.data[svc].up_servers[h .. ":" .. p] then
                                                _M.data[svc].up_servers[h .. ":" .. p] = nil
                                                ngx_log(
                                                    ngx_INFO,
                                                    "MODIFY DELETE up_servers [" .. svc .. "]: " .. h .. ":" .. p
                                                )
                                                hash_server(svc)
                                                ngx_log(ngx_INFO, "hash " .. svc .. " due to DELETE " .. h .. ":" .. p)
                                            end
                                        end
                                    end
                                end
                            ...
                            if bkd == "healthcheck" then
                                ...
                                ngx_log(ngx_INFO, "update healthcheck,", "ret:", ret)
                            else
                                ngx_log(ngx_ERR, "bkd:", bkd, " err:", err)
                            end
                        end
                    end
    ...
    local ok, err = ngx_timer_at(5, watch, conf, nextIndex)
    if not ok then
        ngx_log(ngx_ERR, "Error start watch: ", err)
    end
    return
end
...
function _M.init()
    ...
    if not conf then
        ngx_log(ngx_ERR, "require config.lua")
        return
    end
    -- Load the upstreams from file
    local s = ngx.re.match(conf.etcd_path, "(^/.*?)/{0,}$", "jo")
    if not s then
        ngx_log(ngx_ERR, "etcd_path config error")
        return
    else
        conf.etcd_path = s[1]
    end
    ...
    return request_etcd(
        {
            path = url,
            method = "PUT",
            body = "value=" .. json.encode(health_conf),
            headers = {["Content-Type"] = "application/x-www-form-urlencoded"}
        }
    )
end
